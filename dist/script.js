        ...
        switch (item.nodeName) {
          ...
          case 'INPUT':
            if (item.getAttribute('type') === 'checkbox') {
              i === 0 ? state[prop] = 'Холодное' : state[prop] = 'Тёплое'; // Тернарный оператор, проверяет условие, как if else
...
var forms = function forms(state) {
  ...
  form.forEach(function (item) {
    item.addEventListener('submit', function (e) {
      ...
      var formData = new FormData(item); // Создаем переменную с данными, которые будут отправлены на сервер
...
var tabs = function tabs(headerSelector, tabSelector, contentSelector, activeClass) {
  ...
  header.addEventListener('click', function (e) {
    ...
    if (target && (target.classList.contains(tabSelector.replace(/\./, "")) || target.parentNode.classList.contains(tabSelector.replace(/\./, "")))) {
      // Заменяем точку на пустую строку и проверяем был ли клик на элемент, который относится к tabSelector (к родителю)
      tab.forEach(function (item, i) {
        if (target == item || target.parentNode == item) {
          // Проверяем, был ли клик именно по тому элементу, который перебирается, если да - то мы скрываем всё и выводим этот элемент по счету
          hideTabContent();
          showTabContent(i);
        }
      });
...
var timer = function timer(id, deadline) {
  ...
  var getTimeRemaining = function getTimeRemaining(endtime) {
    var t = Date.parse(endtime) - Date.parse(new Date()),
        // Метод parse принимает в себя строку с датой записанной в определенном формате, и после манипуляций он вернет количество милисекунд с 1976 года, new Data создаёт текущее время
    seconds = Math.floor(t / 1000 % 60),
        // Делим на 1000 мелисекунд и получаем остаток, а Math.floor округляет
